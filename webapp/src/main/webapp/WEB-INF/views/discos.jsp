...
<article class="twelve columns main-content">
	...
	<c:if test="${DISCO.getCreator().length()>0}">
		<h3><em>Created by <a href="<c:url value='/resources/${my:httpEncodeStr(DISCO.getCreator())}'/>">${DISCO.getCreator()}</a></em></h3>
	</c:if>
	<c:if test="${DISCO.getProvGeneratedBy().length()>0}">
		<h3><em>Generated by <a href="<c:url value='/resources/${my:httpEncodeStr(DISCO.getProvGeneratedBy())}'/>">${DISCO.getProvGeneratedBy()}</a></em></h3>
	</c:if>
			...
			<table>
				...
				<c:if test="${properties.size()>0}">
						<c:forEach var="property" items="${properties}">	
							<tr>
								<td><a href="${property.getValue().getPredicateLink()}" title="${property.getValue().getPredicateLink()}">${property.getValue().getPredicateDisplay()}</a></td>
								<td>
